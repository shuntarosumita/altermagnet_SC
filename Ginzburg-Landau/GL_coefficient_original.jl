...
function Parameters(
    ...
    savepath::String = Printf.format(
        Printf.Format("GL_original_t1_%.3f_t2_%.3f_h%s_a_%.3f_n_%.3f_T_%.4f_%s"),
...
function calc_hamiltonian(p::Parameters)::Array{BigComp, 5}
    ...
    h_mol::Matrix{BigComp} = zeros(BigComp, p.nwan, p.nwan)
    if p.h_dir == "f100"
    ...
    for ik3 in 1:p.nk3, ik2 in 1:p.nk2, ik1 in 1:p.nk1
        ...
        ### SOC terms
        gx::BigComp = 0.5im * (1 + cis(k1)) * (1 - cis(-k2))
        gy::BigComp = 0.5im * (1 - cis(k1)) * (1 + cis(-k2))
        h_SOC::Matrix{BigComp} = p.α .* (
            real(gx) .* kron(σ1, σ1) .- imag(gx) .* kron(σ2, σ1)
            .+ real(gy) .* kron(σ1, σ2) .- imag(gy) .* kron(σ2, σ2)
        )

        if p.h_dir == "dxy001"
            h_mol = - 2p.h * sin(k1) * sin(k2) .* kron(σ0, σ3)
        end

        hk[ik1, ik2, ik3, :, :] .= h_hop .+ h_SOC .+ h_mol
    end
...
function calc_hamiltonian_spin_resolved(p::Parameters)
    hk_u = Array{BigComp, 5}(undef, p.nk1, p.nk2, p.nk3, p.nwan÷2, p.nwan÷2)
    hk_d = Array{BigComp, 5}(undef, p.nk1, p.nk2, p.nk3, p.nwan÷2, p.nwan÷2)
    h_mol::Matrix{BigComp} = zeros(BigComp, p.nwan÷2, p.nwan÷2)
    ...
    for ik3 in 1:p.nk3, ik2 in 1:p.nk2, ik1 in 1:p.nk1
        ...
        f1::BigComp = (1 + cis(k1)) * (1 + cis(-k2))
        h_hop::Matrix{BigComp} = (
            - p.t1 .* (real(f1) .* σ1 .- imag(f1) .* σ2)
            .- 2p.t2 * cos(k1) * cos(k2) .* σ0
            .- 2p.t2 * sin(k1) * sin(k2) .* σ3
        )

        hk_u[ik1, ik2, ik3, :, :] .= h_hop .+ h_mol
        hk_d[ik1, ik2, ik3, :, :] .= h_hop .- h_mol
    end
...
function calc_velocity_spin_resolved(p::Parameters)
    vk_u = zeros(BigComp, p.nk1, p.nk2, p.nk3, p.nwan÷2, p.nwan÷2, 3)
    vk_d = zeros(BigComp, p.nk1, p.nk2, p.nk3, p.nwan÷2, p.nwan÷2, 3)
    for ik3 in 1:p.nk3, ik2 in 1:p.nk2, ik1 in 1:p.nk1
        k1::BigFloat = (2π*(ik1-1)) / p.nk1
        k2::BigFloat = (2π*(ik2-1)) / p.nk2

        ### v1 = dH/dk1
        f1_1::BigComp = +1im * cis(k1) * (1 + cis(-k2))
        vk_u[ik1, ik2, ik3, :, :, 1] .= (
            - p.t1 .* (real(f1_1) .* σ1 .- imag(f1_1) .* σ2)
            .+ 2p.t2 * sin(k1) * cos(k2) .* σ0
            .- 2p.t2 * cos(k1) * sin(k2) .* σ3
        )
        vk_d[ik1, ik2, ik3, :, :, 1] .= vk_u[ik1, ik2, ik3, :, :, 1]

        ### v2 = dH/dk2
        f1_2::BigComp = -1im * cis(-k2) * (1 + cis(k1))
        vk_u[ik1, ik2, ik3, :, :, 2] .= (
            - p.t1 .* (real(f1_2) .* σ1 .- imag(f1_2) .* σ2)
            .+ 2p.t2 * cos(k1) * sin(k2) .* σ0
            .- 2p.t2 * sin(k1) * cos(k2) .* σ3
        )
        vk_d[ik1, ik2, ik3, :, :, 2] .= vk_u[ik1, ik2, ik3, :, :, 2]
    end
...
function calc_GL_coefficient(p::Parameters, ϕ::AbstractArray; verbose::Bool=true)
    ...
    # # c2_0: second-order term with zeroth-order derivative
    # c2_0::BigComp = parse(BigComp, "0.0")
    # for iω in 1:p.nω, ik3 in 1:p.nk3, ik2 in 1:p.nk2, ik1 in 1:p.nk1
    #     ### sum(g0p * ϕ * g0h * ϕ')
    #     c2_0 += @views tr(
    #         giωk[iω, ik1, ik2, ik3, :, :] * ϕ[ik1, ik2, ik3, :, :]
    #         * giωk_rev[iω, ik1, ik2, ik3, :, :] * ϕ[ik1, ik2, ik3, :, :]'
    #     )
    # end
    # c2_0 *= p.T / p.nk

    # # c2_1: second-order term with first-order derivative
    # c2_1::Vector{BigComp} = zeros(BigComp, 3)
    # for iω in 1:p.nω, ik3 in 1:p.nk3, ik2 in 1:p.nk2, ik1 in 1:p.nk1, μ1 in 1:3
    #     ### sum(g0p * vμ1 * g0p * ϕ * g0h * ϕ')
    #     c2_1[μ1] += @views tr(
    #         giωk[iω, ik1, ik2, ik3, :, :] * vk[μ1, ik1, ik2, ik3, :, :]
    #         * giωk[iω, ik1, ik2, ik3, :, :] * ϕ[ik1, ik2, ik3, :, :]
    #         * giωk_rev[iω, ik1, ik2, ik3, :, :] * ϕ[ik1, ik2, ik3, :, :]'
    #     )
    # end
    ...
    # # c2_2: second-order term with second-order derivative
    # c2_2::Matrix{BigComp} = zeros(BigComp, 3, 3)
    # for iω in 1:p.nω, ik3 in 1:p.nk3, ik2 in 1:p.nk2, ik1 in 1:p.nk1, μ2 in 1:3, μ1 in 1:3
    #     ### sum(- g0p * vμ1 * g0p * ϕ * g0h * vμ2(-k) * g0h * ϕ' + 2 * g0p * vμ1 * g0p * vμ2 * g0p * ϕ * g0h * ϕ')
    #     c2_2[μ1, μ2] += @views tr(
    #         - (giωk[iω, ik1, ik2, ik3, :, :] * vk[μ1, ik1, ik2, ik3, :, :]
    #         * giωk[iω, ik1, ik2, ik3, :, :] * ϕ[ik1, ik2, ik3, :, :]
    #         * giωk_rev[iω, ik1, ik2, ik3, :, :] * vk_rev[μ2, ik1, ik2, ik3, :, :]
    #         * giωk_rev[iω, ik1, ik2, ik3, :, :] * ϕ[ik1, ik2, ik3, :, :]')
    #         .+ 2 * (giωk[iω, ik1, ik2, ik3, :, :] * vk[μ1, ik1, ik2, ik3, :, :]
    #         * giωk[iω, ik1, ik2, ik3, :, :] * vk[μ2, ik1, ik2, ik3, :, :]
    #         * giωk[iω, ik1, ik2, ik3, :, :] * ϕ[ik1, ik2, ik3, :, :]
    #         * giωk_rev[iω, ik1, ik2, ik3, :, :] * ϕ[ik1, ik2, ik3, :, :]')
    #     )
    # end
...
function calc_GL_coefficient_intraband(p::Parameters, ϕ::AbstractArray; verbose::Bool=true)
    ...
    hk_u, hk_d = calc_hamiltonian_spin_resolved(p)
    ek_u = Array{BigFloat, 4}(undef, p.nk1, p.nk2, p.nk3, p.nwan÷2)
    ek_d = Array{BigFloat, 4}(undef, p.nk1, p.nk2, p.nk3, p.nwan÷2)
    uk_u = Array{BigComp, 5}(undef, p.nk1, p.nk2, p.nk3, p.nwan÷2, p.nwan÷2)
    uk_d = Array{BigComp, 5}(undef, p.nk1, p.nk2, p.nk3, p.nwan÷2, p.nwan÷2)
    for ik3 in 1:p.nk3, ik2 in 1:p.nk2, ik1 in 1:p.nk1
        ek_u[ik1, ik2, ik3, :], uk_u[ik1, ik2, ik3, :, :] = eigen(@view(hk_u[ik1, ik2, ik3, :, :]))
        ek_d[ik1, ik2, ik3, :], uk_d[ik1, ik2, ik3, :, :] = eigen(@view(hk_d[ik1, ik2, ik3, :, :]))
    end
    ek_u .-= p.μ; ek_d .-= p.μ

    # spin-resolved velocity operator
    vk_u, vk_d = calc_velocity_spin_resolved(p)

    ## intraband velocity
    vk_u_intra = Array{BigComp, 5}(undef, p.nk1, p.nk2, p.nk3, p.nwan÷2, 3)
    vk_d_intra = Array{BigComp, 5}(undef, p.nk1, p.nk2, p.nk3, p.nwan÷2, 3)
    for dir in 1:3, a1 in 1:p.nwan÷2, ik3 in 1:p.nk3, ik2 in 1:p.nk2, ik1 in 1:p.nk1
        vk_u_intra[ik1, ik2, ik3, a1, dir] = sum(
            conj(uk_u[ik1, ik2, ik3, ζ1, a1]) * vk_u[ik1, ik2, ik3, ζ1, ζ2, dir] * uk_u[ik1, ik2, ik3, ζ2, a1]
            for ζ2 in 1:p.nwan÷2, ζ1 in 1:p.nwan÷2
        )
        vk_d_intra[ik1, ik2, ik3, a1, dir] = sum(
            conj(uk_d[ik1, ik2, ik3, ζ1, a1]) * vk_d[ik1, ik2, ik3, ζ1, ζ2, dir] * uk_d[ik1, ik2, ik3, ζ2, a1]
            for ζ2 in 1:p.nwan÷2, ζ1 in 1:p.nwan÷2
        )
    end

    ## calculate reversed spin-down eigenvalues/vectors
    ek_d = reverse(
    ...
    uk_d = conj.(
        reverse(
            circshift(uk_d, (-1, -1, -1, 0, 0)),
            dims=(1, 2, 3)
        )
    )
    vk_d_intra = conj.(
        reverse(
            circshift(vk_d_intra, (-1, -1, -1, 0, 0)),
            dims=(1, 2, 3)
        )
    )
    ...
    for a1 in 1:p.nwan÷2, ik3 in 1:p.nk3, ik2 in 1:p.nk2, ik1 in 1:p.nk1
        ϕ_b_intra[ik1, ik2, ik3, a1] = sum(
            conj(uk_u[ik1, ik2, ik3, ζ1, a1]) * ϕ[ik1, ik2, ik3, 2ζ1-1, 2ζ2] * uk_d[ik1, ik2, ik3, ζ2, a1]
            for ζ2 in 1:p.nwan÷2, ζ1 in 1:p.nwan÷2
        )
    end
        ...
        for ik3 in 1:p.nk3, ik2 in 1:p.nk2, ik1 in 1:p.nk1
            for a1 in 1:p.nwan÷2
                c2_2_intra_2 -= (
                    ...
                    * vk_u_intra[ik1, ik2, ik3, a1, 1+i÷3] * vk_d_intra[ik1, ik2, ik3, a1, 1+i÷2]
                    * Matsubara_sum(
...
function main()
    length(ARGS) < 13 && error("usage: julia GL_coefficient_original.jl t1 t2 α h_dir n_fill T SC_type nk1 nk2 nk3 h0 dh h1")
    ...
    elseif SC_type == "d"
        δ::BigFloat = parse(BigComp, "0.0")
        for ik3 in 1:p.nk3, ik2 in 1:p.nk2, ik1 in 1:p.nk1
            k1::BigFloat = (2π*(ik1-1)) / p.nk1
            k2::BigFloat = (2π*(ik2-1)) / p.nk2
            Δfc2_1 = 1.0 - cis(k1) + cis(k1-k2) - cis(-k2)
            ϕ[ik1, ik2, ik3, :, :] .= (
                real(Δfc2_1) .* kron(σ1, im .* σ2) .- imag(Δfc2_1) .* kron(σ2, im .* σ2) # singlet: dxy τ{x,y}
    ...
    for ih in eachindex(hs)
        ...
        open("data_original/$(p.savepath)_c2_0.txt", "a") do data
            println(data,
                "$(p.h) $(real(c2_0))"
            )
        end
        open("data_original/$(p.savepath)_c4_0.txt", "a") do data
            println(data,
                "$(p.h) $(real(c4_0))" # $(real(c4_0_intra))
            )
        end
        # open("data_original/$(p.savepath)_c2_1.txt", "a") do data
        #     println(data,
        #         "$(p.h)"
        #         * prod(" $(imag(c2_n[1][i]))" for i in 1:2)
        #     )
        # end
        open("data_original/$(p.savepath)_c2_2.txt", "a") do data
            println(data,
                "$(p.h)"
        ...
        # open("data_original/$(p.savepath)_c2_3.txt", "a") do data
        #     println(data,
        #         "$(p.h)"
        #         * prod(" $(imag(c2_n[3][i]))" for i in 1:4)
        #     )
        # end
        open("data_original/$(p.savepath)_c2_4.txt", "a") do data
            println(data,
                "$(p.h)"
        ...
        ##### test begin #####
        # open("data_original/$(p.savepath)_test.txt", "a") do data
        #     println(data,
    ...
    # check all the imaginary part is zero
    # max_im = maximum(
    #     [
    #         abs(maximum(imag, c2_0s)),
    #         abs(maximum(imag, c4_0s)),
    #         # abs(maximum(imag, c2_1s)),
    #         # abs(maximum(imag, c2_2s)),
    #     ]
    # )
    # println("maximum imaginary part: $max_im")
...
# function calc_giωk(p::Parameters)::Array{BigComp, 6}
#     # calculate Hamiltonian and its eigenvalues/vectors
#     hk = calc_hamiltonian(p)
#     ek = Array{BigFloat, 4}(undef, p.nk1, p.nk2, p.nk3, p.nwan)
#     uk = Array{BigComp, 5}(undef, p.nk1, p.nk2, p.nk3, p.nwan, p.nwan)
...
#     # chemical potential
#     μ::BigFloat = set_μ(p, ek)
#     println("h = $(p.h): emin = $(minimum(ek)), emax = $(maximum(ek)); μ = $μ")
#     ek .-= μ

#     # calculate giωk using the eigenvalues/vectors
#     giωk = Array{BigComp, 6}(undef, p.nω, p.nk1, p.nk2, p.nk3, p.nwan, p.nwan)
#     for iω in 1:p.nω
#         ωf::BigFloat = (2 * (iω - p.nω÷2 - 1) + 1) * π * p.T

#         for ζ2 in 1:p.nwan, ζ1 in 1:p.nwan, ik3 in 1:p.nk3, ik2 in 1:p.nk2, ik1 in 1:p.nk1
#             giωk[iω, ik1, ik2, ik3, ζ1, ζ2] = sum(
#                 uk[ik1, ik2, ik3, ζ1, j] * conj(uk[ik1, ik2, ik3, ζ2, j]) / (im*ωf - ek[ik1, ik2, ik3, j])
#                 for j in 1:p.nwan
#             )
#         end
#     end
...
# function calc_velocity(p::Parameters)::Array{BigComp, 6}
#     vk = Array{BigComp, 6}(undef, p.nk1, p.nk2, p.nk3, p.nwan, p.nwan, 3)
#     for ik3 in 1:p.nk3, ik2 in 1:p.nk2, ik1 in 1:p.nk1
#         k1::BigFloat = (2π*(ik1-1)) / p.nk1
#         k2::BigFloat = (2π*(ik2-1)) / p.nk2
...
#         ### v1 = dH/dk1
#         f1_1::BigComp = +1im * cis(k1) * (1 + cis(-k2))
#         gx_1::BigComp = -0.5 * cis(k1) * (1 - cis(-k2))
#         gy_1::BigComp = +0.5 * cis(k1) * (1 + cis(-k2))
#         vk[ik1, ik2, ik3, :, :, 1] .= (
#             - p.t1 .* (real(f1_1) .* kron(σ1, σ0) .- imag(f1_1) .* kron(σ2, σ0))
#             .+ 2p.t2 * sin(k1) * cos(k2) .* kron(σ0, σ0)
#             .- 2p.t2 * cos(k1) * sin(k2) .* kron(σ3, σ0)
#             .+ p.α .* (
#                 real(gx_1) .* kron(σ1, σ1) .- imag(gx_1) .* kron(σ2, σ1)
#                 .+ real(gy_1) .* kron(σ1, σ2) .- imag(gy_1) .* kron(σ2, σ2)
#             )
#         )

#         ### v2 = dH/dk2
#         f1_2::BigComp = -1im * cis(-k2) * (1 + cis(k1))
#         gx_2::BigComp = -0.5 * cis(-k2) * (1 + cis(k1))
#         gy_2::BigComp = +0.5 * cis(-k2) * (1 - cis(k1))
#         vk[ik1, ik2, ik3, :, :, 2] .= (
#             - p.t1 .* (real(f1_2) .* kron(σ1, σ0) .- imag(f1_2) .* kron(σ2, σ0))
#             .+ 2p.t2 * cos(k1) * sin(k2) .* kron(σ0, σ0)
#             .- 2p.t2 * sin(k1) * cos(k2) .* kron(σ3, σ0)
#             .+ p.α .* (
#                 real(gx_2) .* kron(σ1, σ1) .- imag(gx_2) .* kron(σ2, σ1)
#                 .+ real(gy_2) .* kron(σ1, σ2) .- imag(gy_2) .* kron(σ2, σ2)
#             )
#         )
#     end
